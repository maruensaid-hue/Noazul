    ...
    <View className="gap-1.5">
      {slices.map((slice) => (
        <View key={slice.categoryId ?? "none"} className="flex-row items-center gap-2">
          ...
          <Text className="flex-1 text-sm text-gray-700">{slice.name}</Text>
          <Text className="text-sm text-gray-500">{Math.round(slice.fraction * 100)}%</Text>
          <Text className="w-20 text-right text-sm font-medium text-gray-900">
